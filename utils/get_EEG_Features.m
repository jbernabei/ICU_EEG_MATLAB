...
p_delta = nanmedian(bandpower(vals',sampleRate,[fcutlow0 fcuthigh0]));

% Theta band power
fcutlow1=4;   %low cut frequency in Hz
fcuthigh1=8;   %high cut frequency in Hz
p_theta = nanmedian(bandpower(vals',sampleRate,[fcutlow1 fcuthigh1]));
...
p_alpha = nanmedian(bandpower(vals',sampleRate,[fcutlow2 fcuthigh2]));
...
p_beta = nanmedian(bandpower(vals',sampleRate,[fcutlow3 fcuthigh3]));
%v_beta = var(bandpower(vals',sampleRate,[fcutlow3 fcuthigh3]));

% Calculate features based on linelength
Line_length = nanmedian(sum(abs(diff(vals'))));
%v_LL = var(sum(abs(diff(vals))));

% Calculate envelope
[yupper,ylower] = envelope(vals');
upper_env = nanmedian(median(yupper));

% Skewness
Skew = nanmedian(nanmedian(skewness(vals')));

% Kurtosis
Kurt = nanmedian(nanmedian(kurtosis(vals')));

% Correlation_Matrix
Synch = nanmedian(nanmedian(corrcoef(vals')));
